...
(crontab -l 2>/dev/null; echo '*/5 * * * * if [ $(free -wm | awk '\''/^Mem:/ { print $8 }'\'') -le 500 ]; then XDG_RUNTIME_DIR=/run/user/$(id -u) notify-send -u normal '\''Available memory: 500 Mb only!'\''; fi') | crontab - # run every 5 minutes, see `man crontab.5`
