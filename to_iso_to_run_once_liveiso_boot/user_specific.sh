...
# if [ $(night read | tail -n1 | awk '{print $4}') -eq $(night read | head -n1 | awk '{print $4}') ]; then # if current brightness of display backlight is same as maximum
night max && night -1 && night -1 # set internal laptop brightness at two logarithmic levels below max
